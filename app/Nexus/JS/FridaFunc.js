...
maximizeButton.addEventListener("click", () => {
    ...
    ipcRenderer.send(isMaximized ? 'frida-maximizeWindow' : 'frida-unmaximizeWindow');
...
function showDropZone() {
    Object.assign(dropZone.style, { height: '100vh', zIndex: '999', visibility: 'visible' });
...
document.body.addEventListener('dragenter', e => {
    ...
    if (items.length > 0 && items[0].kind === 'file') showDropZone();
});
document.body.addEventListener('dragleave', e => {
    e.preventDefault();
    if (!e.relatedTarget || e.relatedTarget.nodeName === "HTML") hideDropZone();
});
...
function handleDrop(e) {
    ...
    fileName.endsWith('.js') ? ScriptPath = filePath : ExePath = filePath;
    if (fileName.endsWith('.exe')) {
        window.CurrentSelectedProcess = { pid: ExePath, platform: 'Windows', mode: 'Spawn' };
    ...
    reader.onload = event => {
        try { editor.setValue(event.target.result || ''); } catch (error) { console.error("error", error); }
    };
...
FridasearchInput.addEventListener('input', function () {
    ...
    throttleTimeout = setTimeout(() => searchProcess(), 10);
});
...
document.querySelector('.Frida-IDE-window-closebtn').addEventListener('click', hiddenFridaProcessWindow);
...
function bufferOutput(line, type = null, withTime = true) {
    ...
    if (!flushTimeout) flushTimeout = setTimeout(flushBufferedOutput, 100);
}
...
function flushBufferedOutput() {
    ...
    const htmlLines = outputBuffer.map(({ line, type, withTime }) => {
        ...
        const html = line.replace(/\[([^\]]+)\]/g, (match, rawTag) => {
            ...
            if (type && tagStyles[type]) return `<span style="${tagStyles[type]}">[${safeTag}]</span>`;
            const matchedStyle = Object.entries(tagStyles).find(([key]) => tagName.endsWith(key));
            if (matchedStyle) return `<span style="${matchedStyle[1]}">[${safeTag}]</span>`;
            return `[${safeTag}]`;
        });
...
function centerElement(elmnt) {
    ...
    if (elmntHeight > mainWindowHeight) elmnt.style.height = mainWindowHeight + 'px';
}
...
document.querySelector('.Frida-IDE-search').addEventListener('click', async function () {
    try {
...
function createOutputPanel() {
    ...
    content.addEventListener('contextmenu', (e) => {
        ...
        let posX = e.clientX;
        let posY = e.clientY;
        if (posX + menuWidth > window.innerWidth - margin) posX = window.innerWidth - menuWidth - margin;
        if (posY + menuHeight > window.innerHeight - margin) posY = window.innerHeight - menuHeight - margin;
    ...
    document.addEventListener('mousemove', (e) => {
        if (isDragging) {
            ...
            if (newHeight < 80) {
                panel.style.transform = 'translateY(100%)';
                setTimeout(() => clearOutputContent(), 300);
                isDragging = false;
                return;
            }
...
FridatoggleButton.addEventListener("change", async function () {
    ...
    if (checked) {
        ...
        fs.writeFile(filePath, code, async (err) => {
            if (err) { showMessage("❌ 保存失败"); return; }
            ...
            window.FridaProc = spawn(batPath, fridaArgs, { cwd: path.dirname(batPath) });
            ...
            window.FridaProc.on("error", (err) => bufferOutput(`[Error] ${err.message}`, 'error'));
            window.FridaProc.on("close", () => bufferOutput(`[✓ Done]`, 'Done'));
        });
    } else {
        try {
...
function addProcessesToList(processes, processTableBody) {
    ...
    window.CurrentSelectedProcess = window.CurrentSelectedProcess || { pid: 0, platform: 'Windows', mode: 'Attach' };
    processes.forEach(({ pid, name }) => {
        ...
        const updateSelection = () => {
            ...
            window.CurrentSelectedProcess = { pid, platform: platformLabel, mode: 'Attach' };
            const modeSelect = document.getElementById('Frida-IDE-modeSelect');
            if (modeSelect) modeSelect.value = 'Attach';
        };
        newRow.addEventListener('click', function (event) {
            if (event.target.type !== 'radio') radioButton.checked = true;
            updateSelection();
        });
...
function runCmdFunction(cmd) {
    return new Promise((resolve, reject) => {
        exec(cmd, (error, stdout, stderr) => {
            error ? reject(new Error(`error: ${stderr}`)) : resolve({ output: stdout });
        });
    });
}
...
async function loadWindowsProcessList() {
    ...
    try {
        ...
        processListWindow.style.display = 'block';
    } catch (error) { showMessage("❌ 进程获取失败"); }
}

var UacExePath = null;
document.querySelector('.Frida-IDE-logo')?.addEventListener('click', () => ipcRenderer.send('open-file-window'));
...
ipcRenderer.on('file-selected', (event, filePath) => {
    if (filePath) {
        ...
        UacExePath = fileName;
        if (fileName.endsWith('.exe')) {
            window.CurrentSelectedProcess = { pid: filePath, platform: 'Windows', mode: 'Spawn' };
        }
    }
});

ipcRenderer.on('set-frida-process', (event, { pid, processName = '' }) => {
    if (pid) {
        window.CurrentSelectedProcess = { pid: pid.toString(), platform: 'Windows', mode: 'Attach' };
        if (FridaPidinput) FridaPidinput.value = pid;
        if (processName && FridanameInput) FridanameInput.value = processName;
    }
});

ipcRenderer.on('run-frida-code', () => {
    const { pid = 0 } = window.CurrentSelectedProcess || {};
    if (!pid || pid === 0) {
        ipcRenderer.send('run-frida-code-response', { success: false, message: '请先设置目标进程' });
        return;
    }
    if (FridatoggleButton) {
        if (FridatoggleButton.checked) {
            ...
            setTimeout(() => {
                FridatoggleButton.checked = true;
                FridatoggleButton.dispatchEvent(new Event("change"));
                ipcRenderer.send('run-frida-code-response', { success: true, message: '正在运行Frida代码' });
            }, 100);
        } else {
            FridatoggleButton.checked = true;
            FridatoggleButton.dispatchEvent(new Event("change"));
        ...
        ipcRenderer.send('run-frida-code-response', { success: false, message: '错误：无法找到运行按钮' });
    }
});

ipcRenderer.on('get-terminal-output', (event, { lines = 50 }) => {
    try {
        const outputContent = logLines.slice(-lines).join('\n');
        ipcRenderer.send('terminal-output-response', { content: outputContent || '终端暂无输出' });
    } catch (error) {
        ipcRenderer.send('terminal-output-response', { content: `获取终端输出失败：${error.message}` });
    }
});

ipcRenderer.on('get-frida-code', () => {
    try {
        const editor = ace.edit('editor');
        const code = editor.getValue();
        ipcRenderer.send('get-frida-code-response', { code: code || '' });
    } catch (error) {
        ipcRenderer.send('get-frida-code-response', { code: `获取代码失败：${error.message}` });
    }
});
