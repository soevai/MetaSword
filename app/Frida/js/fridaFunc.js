...
toggleButton.addEventListener("change", function () {
    var filePath = ScriptPath || `${fridapath}/script/Demo.js`;
    ...
    fs.writeFile(filePath, code, (err) => {
        ...
        switch (state) {
            ...
            case 2: // checked == false, processExec == true
                if (ExePath) {
                    let programName = path.basename(ExePath, path.extname(ExePath));
                    ...
                    command = `powershell -WindowStyle Hidden -Command "Start-Process powershell -ArgumentList '-Command \\"Stop-Process -Name FridaRun -Force\\"' -Verb RunAs -WindowStyle Hidden"`;
                }
                processExec = false;
                break;
            default:
        ...
        fridaProcess = exec(command, (error) => {
            if (error) {
...
function monitorFridaProcess() {
    let checkInterval = setInterval(() => {
        ...
        fastlistProcess.on("close", () => {
            if (!output.includes("FridaRun.exe")) {
                toggleButton.checked = false;
                clearInterval(checkInterval);
                monitoring = false;
            }
        });
        fastlistProcess.on("error", () => {
            toggleButton.checked = false;
            clearInterval(checkInterval);
            monitoring = false;
        });
